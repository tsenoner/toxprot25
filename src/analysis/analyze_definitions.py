...
Usage:
    python -m src.analysis.analyze_definitions
"""
...
def _order_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Classify exclusivity: orders with any venom_tissue/both entries are shared
    vt_orders = set(
        clean.loc[clean["ToxProt definition"].isin(["venom_tissue", "both"]), "Order"].unique()
    )

    orders = pd.DataFrame({"count": order_counts, "Phylum": order_phylum})
    orders["exclusivity"] = orders.index.map(lambda o: "both" if o in vt_orders else "kw_toxin")
...
def plot_grouped_phylum_venn_flow(df: pd.DataFrame, ax: plt.Axes) -> None:
    ...
    # A phylum is keyword-exclusive if it has zero venom_tissue and zero both entries
    kw_group = [p for p in ct.index if ct.loc[p, "venom_tissue"] == 0 and ct.loc[p, "both"] == 0]
    shared_group = [p for p in ct.index if p not in kw_group]
