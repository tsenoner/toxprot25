...
class TestPipelineConfig:
    ...
    def test_validate_missing_data_dir(self, temp_dir):
        ...
        config = PipelineConfig(years=[2020], data_dir=temp_dir / "nonexistent")
        errors = config.validate()
...
class TestYearResult:
    ...
    def test_failure_str(self):
        ...
        result = YearResult(
            year=2020, success=False, stage_completed="download", error="Network error"
        )
        assert "2020" in str(result)
        ...
        with patch("src.data_processing.pipeline.process_xml_file") as mock_parse:
            ...
            with patch("src.data_processing.pipeline.process_toxprot_tsv"):
                with patch("src.data_processing.pipeline.process_dataframe_with_taxonomy"):
                    with patch(
                        "src.data_processing.pipeline.add_habitat_classification"
                    ) as mock_habitat:
...
class TestRunPipeline:
    ...
    def test_skips_completed_years_when_configured(
        self, temp_dir, habitat_mapping_file, habitat_detailed_file
    ):
...
class TestPipelineIntegration:
    ...
    def test_full_pipeline_single_year_mocked(
        self, temp_dir, sample_xml_file, habitat_mapping_file, habitat_detailed_file
    ):
